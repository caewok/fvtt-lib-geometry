...
// Create a simple struct for an ordered line segment, where a is ne of b
...
// Have both f64 and i32 for comparison
...
#[wasm_bindgen]
pub struct OrderedSegmentF64
{
	pub start: OrderedCoordinateF64,
	pub end: OrderedCoordinateF64,
	pub idx: f64, // needed to easily track intersections
}

// #[derive(Debug, Copy, Clone)] // need Copy for the Union
#[derive(Debug, Clone)] // just to see how far we can get without copying
#[wasm_bindgen]
pub struct OrderedSegmentI32
{
	pub start: OrderedCoordinateI32,
	pub end: OrderedCoordinateI32,
	pub idx: i32, // needed to easily track intersections
}

#[derive(Debug, Clone)] // just to see how far we can get without copying
pub struct OrderedSegmentI128
{
	pub start: OrderedCoordinateI128,
	pub end: OrderedCoordinateI128,
	pub idx: i32, // needed to easily track intersections
}


impl OrderedSegmentF64
{
	pub fn new(start: OrderedCoordinateF64, end: OrderedCoordinateF64) -> Self
	{
		Self { start, end, idx: 0. }
	}

	pub fn new_with_idx(start: OrderedCoordinateF64, end: OrderedCoordinateF64, idx: f64) -> Self
	{
		Self { start, end, idx }
	}

	pub fn new_reorder(start: OrderedCoordinateF64, end: OrderedCoordinateF64) -> Self
	{
	...
	pub fn new_reorder_with_index(start: OrderedCoordinateF64, end: OrderedCoordinateF64, idx: f64) -> Self
	{
		let order = start.partial_nw(&end);
		let (start, end) = match order {
			Ordering::Greater => (end, start),
	...
	pub fn coordinates(&self) -> (OrderedCoordinateF64, OrderedCoordinateF64) {
		(self.start, self.end)
	}

	pub fn coords(&self) -> (f64, f64, f64, f64) {
		(self.start.x, self.start.y, self.end.x, self.end.y)
	}

	/// difference in coordinates (∆x, ∆y)
	pub fn delta(&self) -> OrderedCoordinateF64 {
		self.end - self.start
	}
}

impl OrderedSegmentI32
{
	pub fn new(start: OrderedCoordinateI32, end: OrderedCoordinateI32) -> Self
	{
		Self { start, end, idx: 0 }
	}

	pub fn new_with_idx(start: OrderedCoordinateI32, end: OrderedCoordinateI32, idx: i32) -> Self
	{
		Self { start, end, idx }
	}

	pub fn new_reorder(start: OrderedCoordinateI32, end: OrderedCoordinateI32) -> Self
	{
		let order = start.partial_nw(&end);
		let (start, end) = match order {
			Ordering::Greater => (end, start),
	...
	pub fn new_reorder_with_index(start: OrderedCoordinateI32, end: OrderedCoordinateI32, idx: i32) -> Self
	{
		let order = start.partial_nw(&end);
		let (start, end) = match order {
			Ordering::Greater => (end, start),
	...
	pub fn coordinates(&self) -> (OrderedCoordinateI32, OrderedCoordinateI32) {
		(self.start, self.end)
	}
	...
	/// difference in coordinates (∆x, ∆y)
	pub fn delta(&self) -> OrderedCoordinateI32 {
		self.end - self.start
	}
}

impl OrderedSegmentI128
{
	pub fn new(start: OrderedCoordinateI128, end: OrderedCoordinateI128) -> Self
	{
		Self { start, end, idx: 0 }
	}

	pub fn new_with_idx(start: OrderedCoordinateI128, end: OrderedCoordinateI128, idx: i32) -> Self
	{
		Self { start, end, idx }
	}

	pub fn new_reorder(start: OrderedCoordinateI128, end: OrderedCoordinateI128) -> Self
	{
		let order = start.partial_nw(&end);
		let (start, end) = match order {
			Ordering::Greater => (end, start),
	...
	pub fn new_reorder_with_index(start: OrderedCoordinateI128, end: OrderedCoordinateI128, idx: i32) -> Self
	{
		let order = start.partial_nw(&end);
		let (start, end) = match order {
			Ordering::Greater => (end, start),
	...
	pub fn coordinates(&self) -> (OrderedCoordinateI128, OrderedCoordinateI128) {
		(self.start, self.end)
	}

	pub fn coords(&self) -> (i128, i128, i128, i128) {
		(self.start.x, self.start.y, self.end.x, self.end.y)
	}

	/// difference in coordinates (∆x, ∆y)
	pub fn delta(&self) -> OrderedCoordinateI128 {
		self.end - self.start
	}
}


impl PartialEq for OrderedSegmentF64
{
...
impl PartialEq for OrderedSegmentI32
{
	fn eq(&self, other: &Self) -> bool {
		self.start == other.start && self.end == other.end
	}
...
impl PartialEq for OrderedSegmentI128
{
	fn eq(&self, other: &Self) -> bool {
		self.start == other.start && self.end == other.end
	}
...
impl From<OrderedSegmentF64> for OrderedSegmentI32 {
	fn from(item: OrderedSegmentF64) -> Self {
		Self::new_with_idx(item.start.into(), item.end.into(), item.idx as i32)
	}
}

impl From<OrderedSegmentI32> for OrderedSegmentF64 {
	fn from(item: OrderedSegmentI32) -> Self {
		Self::new_with_idx(item.start.into(), item.end.into(), item.idx as f64)
	}
}
...
impl From<OrderedSegmentI32> for OrderedSegmentI128 {
	fn from(item: OrderedSegmentI32) -> Self {
		Self::new_with_idx(item.start.into(), item.end.into(), item.idx)
	}
}
